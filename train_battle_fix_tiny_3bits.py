...
if __name__ == '__main__':
    ...
    parser.add_argument('--algo', type=str, choices={
        'ac', 'mfac', 'mfq', 'il', 'mfqc', 'ilqc', 'milc', 'millc',
        'mill', 'cil', 'hil', 'gil', 'atoc'},
    ...
    log_dir = os.path.join(BASE_DIR, 'data/tmp/{}'.format(
        args.algo + 'fixtiny3b6h'+str(args.seed)+'-'+str(args.len_nei)))
    model_dir = os.path.join(BASE_DIR, 'data/models/{}'.format(
        args.algo+'fixtiny256a6h'+str(args.seed)+'-'+str(args.len_nei)))
    ...
    main_model_dir = os.path.join(BASE_DIR, 'data/models/{}-0'.format(
        args.algo+'fixtiny64a6h'+str(args.seed)+'-'+str(args.len_nei)))

    oppo_model_dir = os.path.join(BASE_DIR, 'data/models/{}-1'.format('il'))
    main_msg_dir = os.path.join(BASE_DIR, 'data/models/{}-msg0'.format(
        args.algo+'fixtiny64a6h'+str(args.seed)+'-'+str(args.len_nei)))
    oppo_msg_dir = os.path.join(
        BASE_DIR, 'data/models/{}-msg1'.format(args.algo))

    models = [spawn_ai(args.algo, sess, env, handles[0], args.algo + '-me', args.max_steps, args.len_nei),
              spawn_ai('il', sess, env, handles[1], 'il-oppo', args.max_steps, args.len_nei)]
    if args.usemsg != 'None':
    ...
    runner = tools.Runner(sess, env, handles, args.map_size, args.max_steps,
                          ...
                          len_nei=args.len_nei, rewardtype=args.rewardtype,
                          crp=crp, is_selfplay=False, is_fix=True)

    for k in range(start_from, start_from + args.n_round):
        eps = magent.utility.piecewise_decay(
            k, [0, 700, 1400, 2000], [1, 0.3, 0.01, 0.01])
        runner.run(eps, k)
