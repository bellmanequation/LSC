...
def data_dict_to_networkx(data_dict):
    ...
    if data_dict[EDGES] is not None and data_dict[EDGES].shape[0] > 0:
        ...
        edges_data = zip(data_dict[SENDERS],
                         data_dict[RECEIVERS], edges_features)
...
def graphs_tuple_to_data_dicts(graph):
    ...
    if graph.receivers is not None:
        graph_of_lists[RECEIVERS] = np.split(
            graph.receivers - offset, edges_splits)
        graph_of_lists[SENDERS] = np.split(
    ...
    for index in range(n_graphs):
        result.append(
            {field: graph_of_lists[field][index] for field in ALL_FIELDS})
    return result
    ...
    for data_dict in data_dicts:
        ...
        for k, v in data_dict.items():
            if v is None:
                ...
                dtype = np.int32 if k in [SENDERS, RECEIVERS, LSENDERS, LRECEIVERS,
                                          HSENDERS, HRECEIVERS, N_NODE, N_EDGE, N_HEDGE] else None
