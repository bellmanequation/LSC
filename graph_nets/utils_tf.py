...
def concat(input_graphs, axis, name="graph_concat"):
    ...
    utils_np._check_valid_sets_of_keys(
        [gr._asdict() for gr in input_graphs])  # pylint: disable=protected-access
...
def repeat(tensor, repeats, axis=0, name="repeat"):
    ...
    with tf.name_scope(name):
        ...
        indicator_matrix = tf.cast(
            tf.expand_dims(range_, 1) >= cumsum, tf.int32)
    ...
    for data_dict in data_dicts:
        ...
        for k, v in data_dict.items():
            if v is None:
                ...
                dtype = tf.int32 if k in [
                    SENDERS, RECEIVERS, N_NODE, N_EDGE] else None
...
def data_dicts_to_graphs_tuple(data_dicts, name="data_dicts_to_graphs_tuple"):
    ...
    utils_np._check_valid_sets_of_keys(
        data_dicts)  # pylint: disable=protected-access
    with tf.name_scope(name):
    ...
    with tf.name_scope(name):
        ...
        for field in {"edges", "senders", "receivers"}:
            ...
            if (field in {"senders", "receivers"} and
                ...
                sliced_graphs_dict[field] = sliced_graphs_dict[field] - \
                    start_node_index
