...
$('#magnet-file-form-conf').val('config.json')
$('#magnet-file-form-map').val('video_1.txt')
function _onresize() {
            ...
            switch (op) {
                ...
                case 'f':
                    ...
                    for (var itAgents = 0, nAgents = data[1].length; itAgents < nAgents; itAgents++) {
                        ...
                        var ch=parseInt(data[1][itAgents][6]);
    ...
    for (var agentID in _mapData[1]) {
        if (_mapData[1].hasOwnProperty(agentID)) {
            ...
            if (gridSize >= 4) {
                ...
                _agentRangeCTX.fill();
                _agentRangeCTX.rotate(-result[2]);
                // if (_mapData[1][agentID][6]==1){
                //     _agentRangeCTX.arc(
                //         originXMaster * gridSize + style['height'] * gridSize / 2,
                //         originYMaster * gridSize + style['width'] * gridSize / 2,
                //         6 * gridSize,
                //         -(parseInt(style['attack-angle'])) / 360 * Math.PI ,
                //         (parseInt(style['attack-angle'])) / 360 * Math.PI ,
                //         false
                //     );
                // }
                console.log(_mapData[1][agentID][5])
                if (gridSize >= 6) {
