...
void GridWorld::set_chs(GroupHandle group, const int *chs){
    std::vector<Agent*> &agents = groups[group].get_agents();
    size_t agent_size = agents.size();
    // LOG(TRACE)<<chs;
    for(int i=0;i<agent_size;i++){
        // LOG(DEB)<<chs[i];
        agents[i]->set_ch(chs[i]);
    }
}
...
void GridWorld::get_info(GroupHandle group, const char *name, void *void_buffer) {
    ...
    }else if(strequ(name,"chs")){
        size_t agent_size = agents.size();
        #pragma omp parallel for
        for (int i = 0; i<agent_size;i++){
            float_buffer[i]=agents[i]->get_ch();
        }
    }
