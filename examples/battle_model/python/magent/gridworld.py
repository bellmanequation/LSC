...
class GridWorld(Environment):
    ...
    def set_chs(self,handle,is_chs):
        assert isinstance(is_chs, np.ndarray)
        assert is_chs.dtype == np.int32
        _LIB.env_set_chs(self.game, handle, is_chs.ctypes.data_as(ctypes.POINTER(ctypes.c_int32)))
    ...
    def get_pos(self, handle):
        ...
        _LIB.env_get_info(self.game, handle, b"pos",
                          buf.ctypes.data_as(ctypes.POINTER(ctypes.c_int32)))
        return buf
    def get_hps(self,handle):
        n=self.get_num(handle)
        buf = np.empty((n,),dtype=np.float32)
        _LIB.env_get_info(self.game, handle, b"hps",
                          buf.ctypes.data_as(ctypes.POINTER(ctypes.c_float)))
        return buf
